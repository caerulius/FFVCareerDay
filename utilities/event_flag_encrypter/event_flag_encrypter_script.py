...
def decrypt(input_code):
    ...
    try:
        byte, byte_data = input_code.replace("$","").split(", ")    
        byte_data = [byte_data]
    except:
        byte, byte_data, byte_data2 = input_code.replace("$","").split(", ")
        byte_data = [byte_data, byte_data2]
    
    if byte == "A2":
        bit = bitsetdecrypt[int('0x' + byte_data[0], 16) % 0x08]
        index = a2index + int('0x' + byte_data[0], 16) // 0x08
        decrypt_str = "%s %s ON" % (hex(index).replace("0x7e","").upper(), bit)
    if byte == "A3":
        bit = bitsetdecrypt[int('0x' + byte_data[0], 16) % 0x08]
        index = a3index + int('0x' + byte_data[0], 16) // 0x08
        decrypt_str = "%s %s OFF" % (hex(index).replace("0x7e","").upper(), bit)
    if byte == "A4":
        bit = bitsetdecrypt[int('0x' + byte_data[0], 16) % 0x08]
    ...
    if byte == "CA":
        ...
        bigindex = int(byte_data[1], base = 16) * 0x20
        index = index + bigindex
        decrypt_str = "%s %s ON" % (hex(index).replace("0x7e","").upper(), bit)
    if byte == "CB":
        bit = bitsetdecrypt[int('0x' + byte_data[0], 16) % 0x08]
        index = cbindex + int('0x' + byte_data[0], 16) // 0x08
        bigindex = int(byte_data[1], base = 16) * 0x20
        index = index + bigindex
        decrypt_str = "%s %s OFF" % (hex(index).replace("0x7e","").upper(), bit)
